...
트레이에는 **전체(요약) 아이콘**이 맨 앞에 뜨고, 그 뒤로 각 배터리가 이어집니다. 아이콘을 **좌클릭/우클릭**하면 그 아이콘에 맞는 상세가 열립니다.

![트레이 아이콘](docs/tray-strip.png)

| 클릭한 아이콘 | 열리는 상세 |
|---|---|
| **전체**(요약 타일) | Claude + Codex **전체** 상세 |
| **Claude 배터리**(C5/CW/CF) | **Claude 섹션만** |
| **Codex 배터리**(X5/XW/X) | **Codex 섹션만** |

![스코프별 메뉴](docs/menus.png)

> 위 메뉴 이미지는 내용·색상을 그대로 옮긴 **미리보기**입니다(실제 창 테두리는 Windows 시스템 메뉴 스타일). `🔄 지금 새로고침`을 누르면 즉시 재조회하고 완료 알림을 띄웁니다.
